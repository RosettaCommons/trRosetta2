...
def calc_lddt_dist(args):
    ...
    pose_j = pose_s[j]
    #
    lddt_1 = float(os.popen(script_dir+"/../lddt/lddt -c %s %s | grep Glob"%(pose_i, pose_j)).readlines()[-1].split()[-1])
    lddt_2 = float(os.popen(script_dir+"/../lddt/lddt -c %s %s | grep Glob"%(pose_j, pose_i)).readlines()[-1].split()[-1])
...
dist = dist + dist.T
#
cluster = AgglomerativeClustering(n_clusters=3, affinity='precomputed', linkage='average').fit(dist)
#
...
candidate_s = glob.glob("%s/candidate_??.pdb"%(trfolder))
candidate_s.sort()
best_tr = list()
n_str = len(candidate_s) // 2
# best among MSA-net
score_s = list()
for cand in candidate_s[:n_str]:
    score = np.mean(np.load(cand[:-4] + "_acc.npz")['lddt'].astype(np.float32))
    score_s.append(score)
idx = np.argmax(score_s)
best_tr.append((score_s[idx], candidate_s[idx]))
...
for cand in candidate_s[n_str:]:
    ...
    score_s.append(score)
idx = np.argmax(score_s)
best_tr.append((score_s[idx], candidate_s[idx+n_str]))

for score, pdb in best_tr:
    i_model += 1
