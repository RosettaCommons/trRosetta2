...
class dataloader:
    ...
    def __init__(self,
        ...
        for p in proteins:
            ...
            if not os.path.exists(path+"native.features.npz"):
                continue
            all_samples = glob.glob(path + "*.npz")
            #
            tr_samples = list()
            CM_samples = list()
            rt_samples = list()
            ...
            for s in all_samples:
                if 'naive' in s:
                    ...
                    CM_samples.append(s)
            #
            np.random.shuffle(CM_samples)
            np.random.shuffle(ha_samples)
            samples = tr_samples + [native_sample] + list(CM_samples[:30]) + list(ha_samples[:10]) # upto 70 decoys
    ...
    def next(self, transform=True, pindex=-1, tr_idx=None):
        ...
        if self.trrosetta_base != None:
            if "naive" in sample: # use corresponding distogram as input
                # [path]/[tar]/naive???.features.npz or [path]/[tar]/naive???_relax.features.npz
                sub_idx = int(sample.split('/')[-1][5])
                tr_fn = self.trrosetta_base + "%s/sub_%d/naive.npz"%(pname, sub_idx)
                distogram_pred = np.load(tr_fn)["dist"]
            elif "tmpl" in sample:
                # [path]/[tar]/tmpl???.features.npz or [path]/[tar]/tmpl???_relax.features.npz
                sub_idx = int(sample.split('/')[-1][4])
                tr_fn = self.trrosetta_base + "%s/sub_%d/templ.npz"%(pname, sub_idx)
                distogram_pred = np.load(tr_fn)["dist"]
            else:
                distogram_pred = np.load(self.base_dist[pname])['dist']
    ...
    def next_old(self, transform=True, pindex=-1, tr_idx=None):
        pname = self.proteins[self.index[self.cur_index]]
        if pindex == -1:
            pindex = np.random.choice(np.arange(self.n[pname]))
        sample = self.samples_dict[pname][pindex]
    ...
    def next_smfull(self, transform=True, pindex=-1):
        pname = self.proteins[self.index[self.cur_index]]
        if pindex == -1:
            pindex = np.random.choice(np.arange(self.n[pname]))
        sample = self.samples_dict[pname][pindex]
