...
def rigidbody_pert(pose):
    init_pert = rosetta.protocols.symmetry.SymDockingInitialPerturbation(True)
    init_pert.apply(pose)

def rigidbody_pert2(pose):
    init_pert = rosetta.protocols.symmetric_docking.SymFoldandDockSlideTrialMover()
    init_pert.apply(pose)

def dock_low_res(pose, sf_low):
    #rigidbody_pert(pose)
    dock_lowres = rosetta.protocols.symmetric_docking.SymDockingLowRes(sf_low)
    dock_lowres.apply(pose)

def dock_high_res(pose, sf_fa):
    dock_highres = rosetta.protocols.symmetric_docking.SymDockingHiRes(sf_fa, sf_fa)
