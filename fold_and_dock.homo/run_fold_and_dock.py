...
def main():
    ...
    init_cmd.append("-symmetry:initialize_rigid_body_dofs")
    init_cmd.append("-ex1 -ex2aro")
    init_cmd.append("-mh:path:scores_BB_BB %s/motif_dock/xh_16_"%scriptdir)
    ...
    symmetrize(pose0,args.SYMM)
    seq_symm = pose0.sequence()
    n_symm = len(seq_symm) // len(seq)
    ...
    rigidbody_pert(pose0) # randomize dof
    ...
    if args.fastrelax == True:
        ...
        # Re-evaluate score w/ contact constraints
        scorefxn_min=create_score_function('ref2015')
        pose0.remove_constraints()
        if rst_user != None:
            rst_user.apply(pose0) 
        scorefxn_min.set_weight(rosetta.core.scoring.atom_pair_constraint, 0.1)
